...
**M (5 points)** — Refazer layout do dashboard + migration para goals.

## Risks

- **R1:** Dashboard existente é completamente substituído — risco de regressão
- **R2:** Gráfico de linha precisa de lib de charts (recharts ou chart.js)
- **R3:** Queries de KPI agregadas podem ser lentas sem índices

## Story

**As a** manager,
**I want** um dashboard com KPI de oportunidades, gráfico de meta mensal e filtros,
**so that** eu possa acompanhar o progresso da equipe em relação às metas de vendas.
...
- Card KPI "Oportunidades" com gráfico de linha
...
- O botão "Editar metas" é renderizado mas não funcional até 3.5

## Acceptance Criteria

1. Filtros de mês, cadências e vendedores funcionais com query params
2. Card KPI: "X Oportunidades em {mês}" com valor real do banco
3. Card meta: "Meta de oportunidades para {mês}: Y"
4. Indicador: "Z% abaixo/acima do previsto até hoje (dia D)"
5. Gráfico de linha: Oportunidades acumuladas vs Meta ao longo do mês
...
9. Botão "Editar metas" renderizado (placeholder até Story 3.5)
...
- [x] **Task 1: Migration — tabelas goals** (AC: 8)
  ...
  - [x] 1.5 Índices: (org_id, month) unique em goals, (org_id, user_id, month) unique em goals_per_user
...
- [x] **Task 2: Dashboard types e services** (AC: 2, 3, 4, 6)
  ...
  - [x] 2.3 Query: oportunidades do mês (leads com status 'qualified' no período)
  - [x] 2.4 Query: meta do mês da tabela goals
  - [x] 2.5 Cálculo: % progresso vs previsto até o dia atual

- [x] **Task 3: Server Actions** (AC: 1, 6)
  - [x] 3.1 Criar `src/features/dashboard/actions/get-dashboard-data.ts`
  - [x] 3.2 `requireAuth()` → Zod validation → query → return ActionResult
...
- [x] **Task 4: Dashboard UI** (AC: 1, 2, 3, 4, 5, 7, 9)
  ...
  - [x] 4.2 Componente DashboardFilters (mês, cadências, vendedores)
  - [x] 4.3 Componente OpportunityKpiCard (KPI grande + meta + %)
  - [x] 4.4 Componente OpportunityChart (gráfico de linha, usar recharts)
  - [x] 4.5 Botão "Editar metas" (placeholder onClick)
  - [x] 4.6 Slots vazios para Ranking (3.3) e Insights (3.4)

- [x] **Task 5: Testes** (AC: 6)
  - [x] 5.1 Testes para dashboard-metrics.service.ts (queries, cálculos)
...
```
src/features/dashboard/                 → REFAZER componentes
src/features/dashboard/components/      → DashboardView.tsx, MetricCard.tsx, etc (substituir)
src/app/(app)/dashboard/page.tsx        → Atualizar para novo DashboardView
supabase/migrations/                    → Nova migration
```

**Dashboard existente:**
- `DashboardView.tsx` — atualmente mostra: Total Leads, leadsByStatus, leadsByPorte, leadsByUf, enrichmentStats, recentImports
- `MetricCard.tsx`, `EnrichmentCard.tsx`, `DistributionCard.tsx`, `RecentImports.tsx`, `PeriodFilter.tsx`
- TODOS serão substituídos pelo novo layout Meetime-style

**Padrões a seguir:**
- Server Actions com `ActionResult<T>` para queries
- `requireAuth()` para autenticação
- `createServerSupabaseClient()` para queries server-side
- Recharts ou similar para gráficos (verificar se já está em dependencies)

**Meetime Reference:** Screenshots 1, 11 — Dashboard hero com KPI, filtros, gráfico de linha
...
| 2026-02-21 | 1.1 | Implementação completa — migration, types, service, action, UI, testes | @dev (Dex) |
...
- DashboardView completely rewritten — old props `{ metrics: DashboardMetrics }` replaced with `{ data: DashboardData; filters: DashboardFilters }`
- Old dashboard components (MetricCard, EnrichmentCard, DistributionCard, RecentImports, PeriodFilter) kept but unused by new DashboardView — will be removed in cleanup story
...
- Cadence/user filtering uses two-step query: enrollment lookup → filtered lead query
