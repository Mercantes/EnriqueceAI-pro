...
**M (5 points)** — 2 charts com queries agregadas + migration para loss_reason_id.

## Risks

- **R1:** Chart de motivos de perda precisa de dados da tabela loss_reasons (3.10) — pode usar seed data
- **R2:** Stacked bar chart precisa de lib de charts robusta

## Story

**As a** manager,
**I want** gráficos de insights mostrando motivos de perda e conversão por origem,
**so that** eu possa tomar decisões estratégicas baseadas em dados.
...
2. Stacked bar chart de conversão por origem (verde = convertido, vermelho = perdido)
3. Dados reais via queries agregadas
4. Empty state quando não há dados suficientes
5. Responsivo
6. Migration: coluna `loss_reason_id` (UUID, nullable, FK → loss_reasons) em `cadence_enrollments`
...
- [x] **Task 1: Migration** (AC: 6)
  ...
  - [x] 1.2 FK → `loss_reasons(id)` — criada tabela `loss_reasons` nesta migration
  - [x] 1.3 **Decisão:** criar tabela `loss_reasons` nesta migration para satisfazer FK, com RLS policies
...
- [x] **Task 3: Server Action** (AC: 3)
  ...
  - [x] 3.2 Aceitar filtros de período (month, cadenceIds, userIds)

- [x] **Task 4: Charts UI** (AC: 1, 2, 4, 5)
  - [x] 4.1 Componente `LossReasonsChart.tsx` — bar chart horizontal com recharts
  - [x] 4.2 Componente `ConversionByOriginChart.tsx` — stacked bar chart
...
- [x] **Task 5: Testes** (AC: 3)
  ...
  - [x] 5.2 Testes para componentes de chart (10 tests — 5 cada)
...
**Nota de dependência:** A tabela `loss_reasons` é formalmente criada na Story 3.10. Porém, para a FK funcionar, esta migration pode criar a tabela básica aqui. Coordenar com Story 3.10 para evitar conflito de migration.

**Alternativa:** Não criar FK agora, apenas coluna TEXT `loss_reason` — mais simples, menos acoplamento. Discutir com @architect.

**Meetime Reference:** Screenshot 1 — 2 cards de insights abaixo dos rankings
...
| 2026-02-21 | 1.1 | Implementação completa: migration, service, action, charts, 30 testes | @dev (Dex) |
...
- TS2322: Recharts Tooltip `formatter` type incompatible — params `value`/`name` can be `undefined`. Fixed with `as never` cast.

### Completion Notes List
- **Decisão origin:** Leads não possuem coluna `source`/`origin`. Usou-se nome da cadência como "origin" no chart de conversão (cada cadência representa um canal de aquisição).
- **Decisão loss_reasons:** Tabela `loss_reasons` criada nesta migration para satisfazer FK. Story 3.10 pode adicionar CRUD sem conflito pois tabela já existe.
- **Decisão status mapping:** `qualified` = convertido, `unqualified`/`archived` = perdido. Leads com status `new`/`contacted` são ignorados (ainda em andamento).
...
| `src/features/dashboard/components/ConversionByOriginChart.tsx` | Created | Stacked bar chart de conversão por cadência |
