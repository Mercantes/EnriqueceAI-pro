...
**M (5 points)** — 3 cards com queries agregadas e breakdown por SDR.

## Risks

- **R1:** Queries de breakdown por SDR podem ser lentas em orgs grandes
- **R2:** Cálculo de "% do previsto" depende de goals (3.2)

## Story

**As a** manager,
**I want** 3 cards de ranking mostrando leads finalizados, atividades realizadas e taxa de conversão por SDR,
...
1. Card "Leads Finalizados": total, % do previsto, meta mês, breakdown por SDR (prospectando/finalizados), média/vendedor
2. Card "Atividades Realizadas": total, % do previsto, meta mês, breakdown por SDR (média diária), média/vendedor
3. Card "Taxa de Conversão": percentual, acima/abaixo meta, meta mês, breakdown por SDR, média/vendedor
...
- [x] **Task 1: Ranking queries service** (AC: 1, 2, 3, 4, 5)
  ...
  - [x] 1.2 Query: leads finalizados por SDR no período (enrollment status = 'completed'/'replied')
  - [x] 1.3 Query: atividades realizadas por SDR (interactions count via enrollment lookup)
  - [x] 1.4 Query: taxa de conversão por SDR (qualified / total leads)
...
- [x] **Task 2: Server Action** (AC: 4, 5)
  ...
  - [x] 2.2 Aceitar filtros de período/cadência/vendedores do parent dashboard

- [x] **Task 3: Ranking UI** (AC: 1, 2, 3, 6)
  - [x] 3.1 Componente `RankingCard.tsx` — reutilizável para os 3 cards
...
**Dependência:** Story 3.2 deve estar concluída (tabelas goals e layout dashboard existem).

**Queries base:**
- Leads finalizados: `cadence_enrollments` com status IN ('completed', 'converted') + JOIN org_members
- Atividades: `interactions` COUNT por user_id no período
- Conversão: leads 'qualified' / total leads por SDR

**Meetime Reference:** Screenshot 1 — 3 cards lado a lado abaixo do hero KPI
...
| 2026-02-21 | 1.1 | Implementação completa — migration, service, action, UI, testes | @dev (Dex) |
...
- enrollment_status enum has no 'converted' value — used 'completed'/'replied' for finished leads
- interactions table has no user_id — used cadence_enrollments.enrolled_by to map interactions to SDR
- SDR display uses user_id.slice(0,8) — proper user display names will come with profiles table
- Conversion rate card: total is a percentage (0-100), not absolute count
- RankingCard is fully reusable — accepts title, icon, unit, data, secondaryLabel
