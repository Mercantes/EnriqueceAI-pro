...
**XL (8 points)** — Drag & drop com dnd-kit, timeline por dia, novo step type.

## Risks

- **R1:** dnd-kit pode exigir SSR handling (Next.js)
- **R2:** Numeração automática ao reordenar é complexa
- **R3:** Step type "research" precisa de migration no enum

## Story

**As a** manager,
**I want** um builder de cadência com timeline por dia e sidebar de atividades arrastáveis,
**so that** eu possa montar cadências visualmente como no Meetime.
...
- Delay automático entre dias (config avançada)
- A/B testing de steps
- Engine de automação de envio
...
Then ela se move para o Dia 2 e a numeração é recalculada em ambos dias

Given que estou visualizando o timeline
When eu clico no header "Dia 2"
Then o conteúdo do Dia 2 colapsa/expande

Given que eu adicionei atividades e clico "Editar Cadência" (salvar)
Then as alterações são salvas no banco com step_order correto por dia
...
- [x] Bottom bar com nome da cadência, Voltar e "Editar Cadência" (salvar)
...
- [x] **Task 1: Migration — step type research** (AC: research type)
...
- [x] **Task 3: Sidebar de atividades** (AC: sidebar + pesquisa)
  ...
  - [x] 3.2 Categorias expandíveis: E-mail, Ligação, Social Point, Pesquisa
  - [x] 3.3 Items arrastáveis (DragOverlay)

- [x] **Task 4: Timeline por dia** (AC: drag, numeração, collapse)
  - [x] 4.1 Componente `CadenceTimeline.tsx`
  - [x] 4.2 Day containers como drop zones
  - [x] 4.3 Numeração automática global (across days)
  - [x] 4.4 Collapsible por dia (Accordion/Collapsible)
  - [x] 4.5 Ícones e cores por tipo (email=azul, ligação=verde, social=roxo, pesquisa=laranja)
...
- [x] **Task 5: Drag & Drop logic** (AC: drag between days)
  ...
  - [x] 5.4 Recalc numeração on drop

- [x] **Task 6: Bottom bar + Salvar** (AC: salvar)
  - [x] 6.1 Componente BottomBar com nome cadência, Voltar, "Editar Cadência"
  - [x] 6.2 Server action para salvar steps (upsert cadence_steps)
  - [x] 6.3 Mapear dias/posições para delay_days e step_order

- [x] **Task 7: Testes**
  - [x] 7.1 Testes para numeração automática (add, remove, reorder)
...
**Source Tree:**
```
src/features/cadences/components/    → Novos componentes de builder
src/features/cadences/actions/       → Update save cadence steps
src/features/cadences/types/         → Adicionar 'research' ao channel type
```
...
**Mapeamento dia/step:** delay_days determina o dia. Dia 1 = delay_days: 0, Dia 2 = delay_days: 1, etc. step_order é a posição dentro do dia.

**Meetime Reference:** Screenshot 5 — Sidebar esquerda com categorias, timeline por dia à direita
...
| 2026-02-21 | 1.1 | Implementation complete — all tasks done | @dev (Dex) |
...
- ActivityTypeSidebar: 4 expandable categories (E-mail, Ligação, Social Point, Pesquisa) with draggable items
...
### File List
- `supabase/migrations/20260221_000008_channel_type_research.sql` — NEW
- `src/features/cadences/types/index.ts` — MODIFIED (ChannelType expanded)
- `src/features/cadences/cadence.schemas.ts` — MODIFIED (channelTypeSchema expanded)
- `src/features/cadences/components/ActivityTypeSidebar.tsx` — NEW
- `src/features/cadences/components/ActivityTypeSidebar.test.tsx` — NEW
- `src/features/cadences/components/CadenceTimeline.tsx` — NEW
- `src/features/cadences/components/CadenceTimeline.test.tsx` — NEW
- `src/features/cadences/components/TimelineBuilder.tsx` — NEW
- `src/features/cadences/actions/save-timeline-steps.ts` — NEW
- `src/features/cadences/actions/save-timeline-steps.test.ts` — NEW
- `src/features/cadences/components/CadenceBuilder.tsx` — MODIFIED (new channel icons, Timeline Builder button)
- `src/features/cadences/components/LeadTimeline.tsx` — MODIFIED (new channel icons/labels)
- `src/features/activities/components/ActivityLeadContext.tsx` — MODIFIED (new channel icons)
- `src/features/activities/components/ActivityExecutionSheetContent.tsx` — MODIFIED (type narrowing for new channels)
- `src/features/templates/components/TemplateListView.tsx` — MODIFIED (new channel icons/labels)
- `src/app/(app)/cadences/[id]/page.tsx` — MODIFIED (searchParams, TimelineBuilder view)
